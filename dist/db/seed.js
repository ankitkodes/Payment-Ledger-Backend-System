...
const ACCOUNT_TYPES = ["Saving", "Current", "Salary"];
// ─── Utility Functions ───
...
function seed() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const existingUsers = yield db.select({ count: sql `count(*)` }).from(User);
            ...
            yield db.transaction((tsx) => __awaiter(this, void 0, void 0, function* () {
                // Insert users
                console.log("  Inserting users...");
                yield tsx.insert(User).values(userRows);
                // Insert accounts
                console.log("  Inserting accounts...");
                yield tsx.insert(Account).values(accountRows);
                ...
                for (let i = 0; i < transactionRows.length; i += 1000) {
                    ...
                    yield tsx.insert(Transaction).values(batch);
                ...
                for (let i = 0; i < ledgerRows.length; i += 1000) {
                    ...
                    yield tsx.insert(LedgerSystem).values(batch);
                ...
                for (let i = 0; i < auditRows.length; i += 1000) {
                    ...
                    yield tsx.insert(Audit_log).values(batch);
                }
            }));
